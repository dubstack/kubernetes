...
func (kl *Kubelet) killPod(pod *api.Pod, runningPod *kubecontainer.Pod, status *kubecontainer.PodStatus, gracePeriodOverride *int64) error {
	...
	// cache the pod cgroup Name for setting the cgroups limits on the pod cgroup later
	pcm := kl.containerManager.NewPodContainerManager()
	var podCgroup string
	reduceMemoryLimts := true
	if pod != nil {
		podCgroup = pcm.GetPodContainerName(pod)
	} else {
		reduceMemoryLimts = false
	}

	// Call the container runtime KillPod method. We
	if err := kl.containerRuntime.KillPod(pod, p, gracePeriodOverride); err != nil {
		return err
	}
	// At this point the pod might not completely free up cpu and memory resources.
	// If the qos cgroup hierarchy has been enabled reduce the resource limits of pods
	// As we cannot delete the pod's cgroup as it might cause the pod's charges to be transferred
	// to the QoS cgroup. There might be various kinds of pod charges at this point.
	// One example is if there exists some tmpfs pages belonging to the pod
	// Hence here only reduce the resource limits for the pod cgroups
	// and  defer the responsibilty of destroying the pod's cgroup to the
	// cleanup method and the housekeeping loop.
	// Only reduce the cpu resources as reducing memory processes might trigger harmful OOM_Kills
	if reduceMemoryLimts {
		minimumCPUShares := int64(2)
		resources := &cm.ResourceConfig{
			CpuShares: &minimumCPUShares,
		}
		pcm.SetResources(podCgroup, resources)
	}
	return nil
}
...
func (kl *Kubelet) syncPod(o syncPodOptions) error {
	...
	// Create Pod's Cgroups. Create pod cgroupsmonly if they don't already exist.
	// EnsureExists would also apply resource parameters to the cgroups
	pcm := kl.containerManager.NewPodContainerManager()
...
// listPodsFromCgroupMounts scans through all the subsytems of pod cgroups
// Get list of pods whose cgroup still exist on the cgroup mounts
func (kl *Kubelet) listPodsFromCgroupMounts() (map[types.UID]string, error) {
	subsystems := kl.containerManager.GetMountedSubsystems()
	qosContainers := kl.containerManager.GetQOSContainers()
...
func (kl *Kubelet) cleanupOrphanedPodCgroups(
	...
	glog.V(3).Infof("XOXOXOXOXOXOXO cleanupOrphanedPodCgroups called, allpods found by the kubelet  %v", allPods)
	// Get list of pods cgroups that still exist on the cgroup mounts
	found, err := kl.listPodsFromCgroupMounts()
	glog.V(3).Infof("XOXOXOXOXOXOXO list of pods found on the cgroup mount  %v", found)
	if err != nil {
