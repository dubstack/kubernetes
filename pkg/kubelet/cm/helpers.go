...
func getLibcontainerCgroupManager(cgroupConfig *CgroupConfig) (*cgroupfs.Manager, error) {
	// get cgroup name
	...
	// get all the mounted subsystems
	cgroupSubsystems, err := getCgroupSubsystems()
	if err != nil {
		return &cgroupfs.Manager{}, fmt.Errorf("Unable to destroy cgroup paths for cgroup %v: %v", name, err)
	}
