...
func NewContainerManager(mountUtil mount.Interface, cadvisorInterface cadvisor.Interface, nodeConfig NodeConfig) (ContainerManager, error) {
	...
	if nodeConfig.CgroupsPerQOS {
		if _, err := os.Stat(nodeConfig.CgroupsRoot); err != nil {
			return &containerManagerImpl{}, fmt.Errorf("invalid configuration: CgroupsRoot doesn't exist : %v", err)
		}
	}
	if nodeConfig.CgroupsPerQOS && nodeConfig.CgroupsRoot == "" {
...
func (cm *containerManagerImpl) setupNode() error {
	...
	if cm.NodeConfig.CgroupsPerQOS {
		qosContainersInfo, err := InitQOS(cm.NodeConfig.CgroupsRoot, cm.subsystems)
		if err != nil {
